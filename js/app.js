...
function glitchCharacter(span, iterations) {
  ...
  span.style.transform = `translate(${rand(-20, 20, true)}px, ${rand(
    -20,
    20,
    true
  )}px)`;
  ...
  span.style.textShadow = `${rand(-2, 2)}px ${rand(-2, 2)}px 2px rgba(${rand(
    0,
    255
  )}, ${rand(0, 255)}, ${rand(0, 255)}, 0.5)`;
...
function glitchText(spans, numberGlitches = 3, delayMin = 0, delayMax = 500) {
  for (var i = 0; i < rand(1, 3, true); i++) {
    ...
    setTimeout(
      glitchCharacter,
      rand(delayMin, delayMax),
      spans[spanIndex],
      rand(0, numberGlitches, true)
    );
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  progressiveImages.forEach((img) => {
    ...
    // Once the WebP version is loaded, start loading the original
    img.addEventListener(
      "load",
      function () {
        // Only load the original if we're still showing the WebP
        if (img.src.includes(".webp")) {
          const originalImage = new Image();
          originalImage.src = originalSrc;

          // When the original loads, replace the WebP
      ...
      { once: true }
    ); // Only trigger this on the first load

    // If WebP fails, switch to original immediately
